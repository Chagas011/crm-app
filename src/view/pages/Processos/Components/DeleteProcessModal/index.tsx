...
export function DeleteProcessModal({ id }: DeleteProcessModalProps) {
  const { mutate } = useDeleteProcess();
  const handleDeleteClient = (processId: string) => {
    mutate({ processId });
  };
    ...
    <Dialog>
      ...
      <DialogContent className="max-w-sm h-70">
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          <p className="text-accent-foreground">Excluir este processo ?</p>
        </div>
